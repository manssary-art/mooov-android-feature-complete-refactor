...
class OrderDetailsScreenContentLoaded extends HookWidget {
  final ValueListenable<OrderModel> order;
  final ValueListenable<bool> isWorking;
  final ValueListenable<GeoPointModel?> userLocation;
  final ValueListenable<String?> translatedDescription;
  final ValueListenable<OrderDetailsDisplayMode> displayMode;
  ...
  Widget build(BuildContext context) {
    ...
    final order = useValueListenable(this.order);
    final isWorking = useValueListenable(this.isWorking);
    final userLocation = useValueListenable(this.userLocation);
    final translatedDescription = useValueListenable(this.translatedDescription);
    final displayMode = useValueListenable(this.displayMode);
          ...
          child: Column(
            children: [
              OrderDetailsPageGallery(
                images: order.images ?? [],
                onImageClicked: (index) => onOrderImageClicked(order.images ?? [], index),
              ),
              SafeArea(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      OrderDetailsTimePicker(
                        ...
                        onWithdrawApplyToOrderClicked: onWithdrawApplyToOrderClicked,
                      ),
                      ...
                      OrderDetailsTranslatableDescription(
                        content: translatedDescription ?? order.description ?? '',
                      ...
                      if (order.pickupAddress != null) ...[
                        ...
                        OrderDetailsAddress(
                          ...
                          onNavigateClicked: () => openOnExternalMap(order.pickupAddress!),
                        ),
                      ],
                      if (order.pickupAddress != null) ...[
                      ...
                      if (order.orderType != OrderType.giveAway && order.deliveryAddresses != null) ...[
                        for (final entry in order.deliveryAddresses!.asMap().entries) ...[
                          const Divider(height: 40),
                          OrderDetailsAddress(
                            address: entry.value,
                            displayMode: displayMode,
                            ...
                            onAddressClicked: copyToClipboard,
                            onNavigateClicked: () => openOnExternalMap(entry.value),
                          ),
                        ],
