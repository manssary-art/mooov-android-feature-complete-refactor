...
class _OrderPaymentScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isSaveCard = ref.watch(saveCardProvider);
    final isSaveCardNotifier = ref.watch(saveCardProvider.notifier);
    final cardNumber = ref.watch(cardNumberProvider);
    final cardNumberNotifier = ref.watch(cardNumberProvider.notifier);
    final cardCVC = ref.watch(cardCVCProvider);
    final cardCVCNotifier = ref.watch(cardCVCProvider.notifier);
    final cardExpirationYear = ref.watch(cardExpirationYearProvider);
    final cardExpirationYearNotifier = ref.watch(cardExpirationYearProvider.notifier);
    final cardExpirationMonth = ref.watch(cardExpirationMonthProvider);
    final cardExpirationMonthNotifier = ref.watch(cardExpirationMonthProvider.notifier);
    ...
    return AnimatedSwitcher(
      ...
      child: switch (step) {
        OrderPaymentStep.selectMethod => HookBuilder(
            key: ValueKey(step),
            builder: (context) {
              return OrderPaymentScreenContentLoadedSelectMethod(
                onNavBackClicked: initialNotifier.onNavBackClicked,
        ...
        OrderPaymentStep.cardInfo => HookBuilder(
            key: ValueKey(step),
            builder: (context) {
              return OrderPaymentScreenContentLoadedCardInfo(
                cardNumber: useUpdateState(cardNumber),
                cvc: useUpdateState(cardCVC),
                expMonth: useUpdateState(cardExpirationMonth),
