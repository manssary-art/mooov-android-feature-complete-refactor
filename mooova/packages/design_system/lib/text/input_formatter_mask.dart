...
enum InputFormatterMask$Type {
  lazy,
  eager,
}

class InputFormatterMask implements TextInputFormatter {
  final InputFormatterMask$Type type;
  ...
  ///  - [InputFormatterMask$Type.lazy] (default): autocomplete unfiltered characters once the following filtered character is input.
  ...
  InputFormatterMask({
    ...
    String? initialText,
    this.type = InputFormatterMask$Type.lazy,
  }) {
  ...
  TextEditingValue updateMask({String? mask, Map<String, RegExp>? filter}) {
    ...
    clear();
    return formatEditUpdate(TextEditingValue.empty,
  ...
  String maskText(String text) {
    return InputFormatterMask(mask: _mask, filter: _maskFilter, initialText: text).getMaskedText();
  }

  /// Unmask some text
  String unmaskText(String text) {
    return InputFormatterMask(mask: _mask, filter: _maskFilter, initialText: text).getUnmaskedText();
  }
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    while (maskPos < mask.length) {
      ...
      } else if (!isMaskChar && !curTextInRange && type == InputFormatterMask$Type.eager) {
        curTextInRange = true;
      }
      ...
      if (isMaskChar && curTextInRange && curTextChar != null) {
        ...
        if (type == InputFormatterMask$Type.lazy || lengthRemoved > 0 || currentResultSelectionLength > 0) {
          nonMaskedCount++;
        }
      }
